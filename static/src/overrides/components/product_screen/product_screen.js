...
import { GlobalDiscountPopupWidget } from "@pos_dual_discount/apps/popups/GlobalDiscountPopupWidget/GlobalDiscountPopupWidget";
    ...
    _setValue(val) {
        ...
        if (order.get_selected_orderline()) {
            if (mode == "discount") {
                ...
                var discount = ((order.get_selected_orderline().get_display_price() * order.get_selected_orderline().quantity - current_price) /

                (order.get_selected_orderline().price * order.get_selected_orderline().quantity)) * 100;
